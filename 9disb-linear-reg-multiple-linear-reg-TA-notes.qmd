...
For all three of the predictors of interest, the relationship with log-Ozone seems much more linear than with natural scale Ozone.
...
The natural scale Ozone model certainly violates normality and constant variance conditions. The log scale Ozone model seems to more closely satisfy the conditions.
...
In the natural scale, $b_3$ represents the expected change in Ozone for a one degree change in temperature. For the log scale, $b_3$ represents that for a one degree change in temperature, the expected Ozone is multiplied by $e^{b_3}$.
