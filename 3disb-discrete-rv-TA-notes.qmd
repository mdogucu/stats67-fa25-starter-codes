...
4.14 on page 166 (a-c)

_Remember the book uses a different definition for geometric distribution_

__4.14 Defective Rate.__ A machine that produces a special type of transistor (a component of computers) has a 2\% defective rate. The production is considered a random process where each transistor is independent of the others. 
...
Let $X$ be a discrete random variable representing the number of transistors before the first defective transistor. We can say that $X \sim Geom(0.02)$.
...
(b) $P(X \geq 100) = 1 - P(X < 100) = 1 - P(X \leq 99) = 1 - F(99)$

```{r, include=T}

#1 - F(99)
1 - pgeom(q = 99, prob = 0.02)

```

(c) $E[X] = \frac{1-0.02}{0.02}$, $Var(X) = \frac{1-0.02}{0.02^2}$
...
4.22 on page 176 (a-c)

__4.22 Arachnophobia.__ A Gallup Poll found that 7\% of teenagers (ages 13 to 17) suffer from arachnophobia and are extremely afraid of spiders. At a summer camp there are 10 teenagers sleeping in each tent. Assume that these 10 teenagers are independent of each other.
...
Let $X$ be a discrete random variable representing the number of teenagers at the summer camp suffering from arachnophobia. We can say that $X \sim Binom(10, 0.07)$.

(a) $P(X \geq 1) = 1 - P(X = 0) = 1 - f(0)$

```{r, include=T}

#f(0)
dbinom(x = 0, size = 10, prob = 0.07)
...
#f(0) + f(1)
dbinom(x = 0, size = 10, prob = 0.07) + dbinom(x = 1, size = 10, prob = 0.07)
```
...
4.32 on page 185 (a,b,d)

__4.32 Stenographer's typos.__ A very skilled court stenographer makes one typographical error (typo) per hour on average.
...
(a) Let $X$ be a discrete random variable representing the number of typos the stenographer makes in one hour. We can say that $X \sim Poisson(1)$.

(b) The mean and standard deviation of a $Poisson(1)$ random variable are both 1.
...
# f(0) + f(1) + f(2)
dpois(x = 0, lambda = 1) + dpois(x = 1, lambda = 1) + dpois(x = 2, lambda = 1)
